...
import { fetchMyProfile, upsertMyProfile } from '../lib/players'
...
export default function PlayerRegister() {
  ...
  useEffect(() => {
    ...
    fetchMyProfile(session.user.id).then((profile) => {
      if (profile) {
        setName(profile.in_game_name)
        setRegion(profile.region)
        setOpenfrontId(profile.openfront_id)
      }
    })
  }, [session])
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          <Link to="/profile" className="btn-ghost">{t.profile.title}</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <p className="text-sm text-signal-green mb-6">✓ {t.playerRegister.loggedInAs} {discordDisplayName(session)}</p>
